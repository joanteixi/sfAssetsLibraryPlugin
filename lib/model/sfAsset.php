...
class sfAsset extends BasesfAsset
{
    ...
    /**
     * Return the size (width x height)
     * @param <type> $asset
     * @return <type>
     */
    public function getSize($type = 'full')
    {
        list($w, $h, $type, $attr) = getimagesize($this->getFullPath($type));
        return $w.' x '.$h;
    }
     ...
     *
     */
    public function getWidth($type='full')
    {
        list($w, $h, $type, $attr) = getimagesize($this->getFullPath($type));
        return $w;
    ...
    /**
     * Return fle size in Kb of file. IF asst is "full" it returns the filesize saved in db, also,
     * returns the size of image.
     * @param <type> $type
     * @return <type>
     */
    public function getFilesize($type = 'full')
    {
        if ($type == 'full')
        {
        ...
        {
            return sfAssetsLibraryTools::getFileSize($this->getFullPath($type));
        }
    }
    /**
...
/*
sfPropelBehavior::add('sfAsset', array('act_as_sortable' => array(
  'column' => 'rank',
  'key' => 'folder_id'
)));
*/