...
class sfAssetsLibraryTools {
  ...
  /**
   * Retorna el file size en el format demanat
   * @param string $format
   * @return string
   */
   ...
   * Options : shave_all
   * Recommended when  "image source HEIGHT" < "image source WIDTH"
   */
  public static function createThumbnail($source, $dest, $width, $height, $shave_all = false) {
    if (class_exists('sfImage') and file_exists($source)) {
      ...
      if ($shave_all) {
        //he fet servir el imageTransform. Permet thumbnails sense modificar la ratio.
        try {
          $thumbnail = new sfImage($source);
          $thumbnail->thumbnail($width, $height, 'center'); //opcions: 'fit', 'center', 'scale', etc, etc: center retalla la imatge pel mig.
          $thumbnail->saveAs($dest, $mime);
        } catch (sfAssetException $e) {
          var_dump($e);
          throw new sfAssetException($e);
        }
        return true;
      } else {
        ...
        try {
          $thumbnail = new sfImage($source);
          $thumbnail->thumbnail($width, $newHeight, 'fit');
          $thumbnail->saveAs($dest, 'image/jpeg');
          return true;
        } catch (sfAssetException $e) {
                    var_dump($e);
   ...
   * @param integet $h height
   */
  public static function scaleImage($source, $dest, $w, $h) {
    if (class_exists('sfImage') and file_exists($source)) {
  ...
  public static function createImageCut($source, $dest, $width, $height, $x, $y, $th = false) {
    if (class_exists('sfImage') and file_exists($source)) {
      $adapter = 'sfGDAdapter';
      $mime = 'image/jpeg';
      list($w, $h, $type, $attr) = getimagesize($source);
      ...
      if ($th) {
        $thumbnail->resize('86', '86');
      }
      $thumbnail->saveAs($dest, $mime);
      return true;
    } else {
      throw new sfException('No es pot trobar la classe sfImage o el fitxer no existeix: ' . $source);
    }
  }
